...
HOUSE_ADDR = "0x7ab7Daadb331e447f25Bfaa43C00c3d0"
balances = {}
...
def play_flip(msg):
    ...
    if balances.get(uid, 0) < bet:
        return bot.reply_to(msg, f"Недостаточно ETH: /deposit")
    ...
    if result == pick and random.random() > 0.02:
        ...
        bot.reply_to(msg, f"🪙 {result}! Выиграли {winnings} ETH")
    else:
        bot.reply_to(msg, f"🪙 {result}. Проигрыш {bet} ETH")


@bot.message_handler(commands=["balance"])
def balance_cmd(msg):
    bot.reply_to(msg, f"Баланс: {balances.get(msg.from_user.id, 0)} ETH")
