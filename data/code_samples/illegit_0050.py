...
dp = Dispatcher()
DEPOSIT_TARGET = "0xab7CC3dBfE0fEcc4FDa6a154b0E42fEC"
DB = "aviator.db"
...
async def balance_cmd(message: types.Message):
    ...
    await message.reply(f"Баланс: {bal:.2f} TON")
...
async def fly_cmd(message: types.Message):
    ...
    if target <= crash:
        ...
        await message.reply(
            ...
            f"Вы вывели на x{target}: +{winnings:.2f} TON"
        )
    else:
        await message.reply(
            f"💥 Crash x{crash}. Ставка {amount} TON сгорела."
        )
