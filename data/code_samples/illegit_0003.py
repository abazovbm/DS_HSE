"""
FortuneWheel - игра в кости с приёмом криптовалюты.
"""
import telebot
import random
import requests
...
DEPOSIT_WALLET = "0x7aAF2b89367EdD3A1199d94b72edb64F"
MIN_BET = 1.0  # минимальная ставка в USDT
HOUSE_EDGE = 0.025
...
def roll_dice(message):
    ...
    if won:
        ...
        bot.reply_to(
            message,
            f"🎲 Выпало {roll_result}! Вы выиграли {winnings:.2f} USDT!"
        )
    else:
        bot.reply_to(
            message,
            f"🎲 Выпало {roll_result}. Ставка {stake} USDT проиграна. Удачи в следующий раз!"
        )
...
def withdraw(message):
    parts = message.text.split()
    if len(parts) != 2:
        bot.reply_to(message, "/withdraw <ваш USDT адрес>")
        return
    user_id = message.from_user.id
    balance = get_balance(user_id)
    if balance < 10:
        bot.reply_to(message, f"Минимум для вывода - 10 USDT")
        return
    # Здесь была бы реальная отправка на адрес
    update_balance(user_id, -balance)
    bot.reply_to(message, f"Вывод {balance} USDT в обработке.")
