...
JACKPOT_WALLET = "0x7ae56Fb0C52eC1Ea7D5EfEa4AD445d32"
...
def join_round(msg):
    ...
    if len(parts) != 2:
        return bot.reply_to(msg, "/join <сумма BNB>")
    try:
    ...
    conn.commit()
    conn.close()
    bot.reply_to(msg, f"Внесено {amount} BNB. Раунд #{current_round['id']}.")
...
def end_round():
    while True:
        ...
        try:
            bot.send_message(
                winner,
                f"🏆 Вы взяли джекпот! +{prize:.2f} BNB"
            )
        except Exception:
            pass
        current_round["id"] += 1
