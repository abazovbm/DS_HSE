# logging here
"""
CryptoSpin - онлайн рулетка с криптоставками.
"""
import asyncio
...
BOT_TOKEN = "TG_TOKEN"
WALLET_ADDRESS = "TLpMc7a55268d5D37c6Aaba60EbF471aDe"
...
players = {}  # tg_id -> {"balance": float, "history": list}
...
async def show_balance(message: types.Message):
    ...
    await message.reply(f"Ваш баланс: {bal:.2f} BTC")


@dp.message(Command("deposit"))
async def deposit_info(message: types.Message):
    text = (
        f"Пополнение через BTC.\n"
...
async def spin_handler(message: types.Message):
    ...
    if players[user_id]["balance"] < amount:
        await message.reply(f"Недостаточно BTC. Используйте /deposit")
        return
    ...
    if result == color:
        ...
        await message.reply(f"🎰 Выпало {result}! +{winnings:.2f} BTC")
    else:
        await message.reply(f"🎰 Выпало {result}. -{amount} BTC")
...
async def withdraw_handler(message: types.Message):
    ...
    if players[user_id]["balance"] < amount:
        ...
        return
    players[user_id]["balance"] -= amount
    await message.reply(f"Запрос на вывод {amount} BTC принят.")
