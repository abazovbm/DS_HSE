...
HOUSE_WALLET = "0x7a14Bc33a8bF46523fCf0C7257be5F08"
DB_FILE = Path("mines_state.json")

GRID_SIZE = 25  # 5x5
games = {}  # tg_id -> {"mines": set, "opened": set, "bet": float, "mult": float}
...
def start_game(message):
    ...
    if balances.get(str(uid), 0) < bet_stake:
        return bot.reply_to(message, f"Пополните баланс BTC: /deposit")
    ...
    bot.reply_to(
        message,
        f"Игра начата. Ставка: {bet_amount} BTC, мин: {mines_n}.\n"
...
def open_cell(message):
    ...
    if idx in game["mines"]:
        del games[uid]
        return bot.reply_to(message, f"💣 Мина! Ставка {game['bet']} BTC сгорела.")
    ...
    bot.reply_to(
        ...
        f"Возможный вывод: {game['bet'] * game['mult']:.2f} BTC"
    )
...
def cashout(message):
    ...
    save_balances(balances)
    del games[uid]
    bot.reply_to(message, f"💰 Вывод: {winnings:.2f} BTC")
