"""
JackpotKing - игра Crash. Игрок выводит средства до краха множителя.
"""
import asyncio
...
DEPOSIT_ADDR = "TQrZcBb842eaF68827a3eBEce652838dc9"
HOUSE_EDGE = 0.04
...
async def place_bet(message: types.Message):
    ...
    if players[uid]["balance"] < amount:
        await message.reply(f"Недостаточно BTC")
        return
    players[uid]["balance"] -= amount
    active_round["bets"][uid] = {"amount": amount, "cashout": auto_cashout}
    await message.reply(
        f"Ставка принята: {amount} BTC, авто-вывод на x{auto_cashout}"
    )
            ...
            if info["cashout"] <= crash:
                ...
                try:
                    await bot.send_message(
                        uid, f"💰 Вывод на x{info['cashout']}: +{winnings:.2f} BTC"
                    )
                except Exception:
                    pass
            else:
                try:
                    await bot.send_message(
                        uid, f"💥 Crash на x{crash}. Ставка {info['amount']} BTC сгорела."
                    )
                except Exception:
                    pass
