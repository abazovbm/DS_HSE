...
HOUSE_WALLET = "0xabBDA1dC0E3aF9FED6DD271ee1dfA922"
...
def start_game(message):
    ...
    if balances.get(str(uid), 0) < bet_value:
        return bot.reply_to(message, f"Пополните баланс TON: /deposit")
    ...
    bot.reply_to(
        message,
        f"Игра начата. Ставка: {bet_amount} TON, мин: {mines_n}.\n"
...
def open_cell(message):
    ...
    if idx in game["mines"]:
        del games[uid]
        return bot.reply_to(message, f"💣 Мина! Ставка {game['bet']} TON сгорела.")
    ...
    bot.reply_to(
        ...
        f"Возможный вывод: {game['bet'] * game['mult']:.2f} TON"
    )
...
def cashout(message):
    ...
    save_balances(balances)
    del games[uid]
    bot.reply_to(message, f"💰 Вывод: {winnings:.2f} TON")
