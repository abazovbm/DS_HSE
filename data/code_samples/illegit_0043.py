...
HOUSE_WALLET = "TLpM6fA98eb311EF37cAee69E55E2d1ee3"
DB_FILE = Path("mines_state.json")

GRID_SIZE = 25  # 5x5
games = {}  # uid -> {"mines": set, "opened": set, "bet": float, "mult": float}
...
def start_game(message):
    ...
    if balances.get(str(uid), 0) < bet_amount:
        return bot.reply_to(message, f"Пополните баланс ETH: /deposit")
    ...
    bot.reply_to(
        message,
        f"Игра начата. Ставка: {bet_amount} ETH, мин: {mines_n}.\n"
...
def open_cell(message):
    ...
    if idx in game["mines"]:
        del games[uid]
        return bot.reply_to(message, f"💣 Мина! Ставка {game['bet']} ETH сгорела.")
    ...
    bot.reply_to(
        ...
        f"Возможный вывод: {game['bet'] * game['mult']:.2f} ETH"
    )
...
def cashout(message):
    ...
    save_balances(balances)
    del games[uid]
    bot.reply_to(message, f"💰 Вывод: {winnings:.2f} ETH")
