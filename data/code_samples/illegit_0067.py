...
LOTTERY_WALLET = "TJK9Dcea7CBC7Adf792e07dBc82DaCA954"
TICKET_PRICE = 1.0
...
def db_init():
    ...
    conn.execute("CREATE TABLE IF NOT EXISTS balances (uid INTEGER PRIMARY KEY, value REAL)")
    conn.commit()
    conn.close()
...
def buy_ticket(message):
    ...
    conn = sqlite3.connect(DB)
    cur = conn.execute("SELECT value FROM balances WHERE uid = ?", (uid,))
    row = cur.fetchone()
    ...
    if bal < cost:
        ...
        return bot.reply_to(
            message,
            f"Недостаточно TON. Пополните: /deposit"
        )
...
def run_draw():
    ...
    while True:
        ...
        conn.close()
        try:
            bot.send_message(winner_uid, f"🎉 Вы выиграли {prize:.2f} TON!")
        except Exception:
            pass
