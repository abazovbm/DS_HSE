...
HOUSE_ADDR = "TQrZf6A0B2DADAc5BB6eE8FF6c05dBDCEf"
balances = {}
...
def play_flip(msg):
    ...
    if balances.get(uid, 0) < bet:
        return bot.reply_to(msg, f"Недостаточно TRX: /deposit")
    ...
    if result == pick and random.random() > 0.02:
        ...
        bot.reply_to(msg, f"🪙 {result}! Выиграли {winnings} TRX")
    else:
        bot.reply_to(msg, f"🪙 {result}. Проигрыш {bet} TRX")


@bot.message_handler(commands=["balance"])
def balance_cmd(msg):
    bot.reply_to(msg, f"Баланс: {balances.get(msg.from_user.id, 0)} TRX")
