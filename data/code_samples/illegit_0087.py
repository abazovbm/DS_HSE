...
JACKPOT_WALLET = "TJK97cE502DF4ce3B7eaAD86d33B7dcC13"
...
def join_round(msg):
    ...
    if len(parts) != 2:
        return bot.reply_to(msg, "/join <сумма BTC>")
    try:
    ...
    conn.commit()
    conn.close()
    bot.reply_to(msg, f"Внесено {amount} BTC. Раунд #{current_round['id']}.")
...
def end_round():
    while True:
        ...
        try:
            bot.send_message(
                winner,
                f"🏆 Вы взяли джекпот! +{prize:.2f} BTC"
            )
        except Exception:
            pass
        current_round["id"] += 1
