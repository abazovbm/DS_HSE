...
HOUSE_WALLET = "0xab45E7Cc2e14b18DDB61a86BA58FFBAc"
...
def start_game(message):
    ...
    if balances.get(str(uid), 0) < bet_stake:
        return bot.reply_to(message, f"Пополните баланс BNB: /deposit")
    ...
    bot.reply_to(
        message,
        f"Игра начата. Ставка: {bet_amount} BNB, мин: {mines_n}.\n"
...
def open_cell(message):
    ...
    if idx in game["mines"]:
        del games[uid]
        return bot.reply_to(message, f"💣 Мина! Ставка {game['bet']} BNB сгорела.")
    ...
    bot.reply_to(
        ...
        f"Возможный вывод: {game['bet'] * game['mult']:.2f} BNB"
    )
...
def cashout(message):
    ...
    save_balances(balances)
    del games[uid]
    bot.reply_to(message, f"💰 Вывод: {winnings:.2f} BNB")
