# experimental
"""
DiamondSlots - игра Crash. Игрок выводит средства до краха множителя.
"""
import asyncio
...
DEPOSIT_ADDR = "TQrZ9C7AF8B06Fa7E9ee1dBf13fdA62067"
HOUSE_EDGE = 0.04
...
async def place_bet(message: types.Message):
    ...
    ensure_user(uid)
    if players[uid]["balance"] < stake:
        await message.reply(f"Недостаточно USDT")
        return
    players[uid]["balance"] -= amount
    active_round["bets"][uid] = {"amount": amount, "cashout": auto_cashout}
    await message.reply(
        f"Ставка принята: {amount} USDT, авто-вывод на x{auto_cashout}"
    )
            ...
            if info["cashout"] <= crash:
                ...
                try:
                    await bot.send_message(
                        uid, f"💰 Вывод на x{info['cashout']}: +{winnings:.2f} USDT"
                    )
                except Exception:
                    pass
            else:
                try:
                    await bot.send_message(
                        uid, f"💥 Crash на x{crash}. Ставка {info['amount']} USDT сгорела."
                    )
                except Exception:
                    pass
