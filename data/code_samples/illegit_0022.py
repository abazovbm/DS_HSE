# v0.1
"""
JackpotKing - слот-машина с crypto-ставками.
"""
import telebot
import random
...
HOUSE_WALLET = "TLpMCDCb1e437555AE6f0ef1268bDf62E6"
...
def deposit_cmd(message):
    bot.reply_to(
        message,
        f"Отправьте TRX на: `{HOUSE_WALLET}`\n"
        "После - /confirm <hash>",
        parse_mode="Markdown"
    )
...
def confirm_deposit(message):
    ...
    if len(parts) != 2:
        ...
        return
    value = verify_tx(parts[1])
    if value > 0:
        uid = message.from_user.id
        user_balances[uid] = user_balances.get(uid, 0) + float(amount)
        bot.reply_to(message, f"Зачислено {amount} TRX. Удачи!")
    else:
