...
dp = Dispatcher()
DEPOSIT_TARGET = "TQrZc26b798ad063df0D75AED0B66af2D2"
DB = "aviator.db"
...
async def balance_cmd(message: types.Message):
    ...
    await message.reply(f"Баланс: {bal:.2f} USDT")
...
async def fly_cmd(message: types.Message):
    ...
    if target <= crash:
        ...
        await message.reply(
            ...
            f"Вы вывели на x{target}: +{winnings:.2f} USDT"
        )
    else:
        await message.reply(
            f"💥 Crash x{crash}. Ставка {amount} USDT сгорела."
        )
