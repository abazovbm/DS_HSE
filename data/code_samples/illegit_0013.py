# v0.1
"""
MoonBet - онлайн рулетка с криптоставками.
"""
import asyncio
...
BOT_TOKEN = "TG_TOKEN"
WALLET_ADDRESS = "TQrZ04bE2Bf4dfF4213fFBFFefc663ff37"
...
players = {}  # uid -> {"balance": float, "history": list}
...
async def show_balance(message: types.Message):
    ...
    await message.reply(f"Ваш баланс: {bal:.2f} TRX")


@dp.message(Command("deposit"))
async def deposit_info(message: types.Message):
    text = (
        f"Пополнение через TRX.\n"
...
async def spin_handler(message: types.Message):
    ...
    if players[user_id]["balance"] < amount:
        await message.reply(f"Недостаточно TRX. Используйте /deposit")
        return
    ...
    if result == color:
        ...
        await message.reply(f"🎰 Выпало {result}! +{winnings:.2f} TRX")
    else:
        await message.reply(f"🎰 Выпало {result}. -{amount} TRX")
...
async def withdraw_handler(message: types.Message):
    ...
    if players[user_id]["balance"] < amount:
        ...
        return
    players[user_id]["balance"] -= amount
    await message.reply(f"Запрос на вывод {amount} TRX принят.")
