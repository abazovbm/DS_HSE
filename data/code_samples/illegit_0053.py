...
dp = Dispatcher()
DEPOSIT_TARGET = "0xab31EF7FDf9ed917F9cEbe628ca101Cd"
DB = "aviator.db"
...
def init_db():
    ...
    db_query("""CREATE TABLE IF NOT EXISTS bets (
        ...
        uid INTEGER, value REAL, target REAL, status TEXT
    )""")
...
async def balance_cmd(message: types.Message):
    ...
    await message.reply(f"Баланс: {bal:.2f} TRX")
...
async def fly_cmd(message: types.Message):
    ...
    if target <= crash:
        ...
        await message.reply(
            ...
            f"Вы вывели на x{target}: +{winnings:.2f} TRX"
        )
    else:
        await message.reply(
            f"💥 Crash x{crash}. Ставка {amount} TRX сгорела."
        )
