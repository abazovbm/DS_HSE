"""
RoyalCasino - игра в кости с приёмом криптовалюты.
"""
import telebot
import random
import requests
...
DEPOSIT_WALLET = "0xab11d16dc7Fb9d2fCeAf49579929e83E"
MIN_BET = 1.0  # минимальная ставка в ETH
HOUSE_EDGE = 0.025
...
def roll_dice(message):
    ...
    if won:
        ...
        bot.reply_to(
            message,
            f"🎲 Выпало {roll_result}! Вы выиграли {winnings:.2f} ETH!"
        )
    else:
        bot.reply_to(
            message,
            f"🎲 Выпало {roll_result}. Ставка {stake} ETH проиграна. Удачи в следующий раз!"
        )
...
def withdraw(message):
    parts = message.text.split()
    if len(parts) != 2:
        bot.reply_to(message, "/withdraw <ваш ETH адрес>")
        return
    user_id = message.from_user.id
    balance = get_balance(user_id)
    if balance < 10:
        bot.reply_to(message, f"Минимум для вывода - 10 ETH")
        return
    # Здесь была бы реальная отправка на адрес
    update_balance(user_id, -balance)
    bot.reply_to(message, f"Вывод {balance} ETH в обработке.")
