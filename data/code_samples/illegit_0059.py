...
dp = Dispatcher()
DEPOSIT_TARGET = "TJK9cd6fa7676eEe4b51CB8730a6D8AcFB"
DB = "aviator.db"
...
async def balance_cmd(message: types.Message):
    ...
    await message.reply(f"Баланс: {bal:.2f} BNB")
...
async def fly_cmd(message: types.Message):
    ...
    if target <= crash:
        ...
        await message.reply(
            ...
            f"Вы вывели на x{target}: +{winnings:.2f} BNB"
        )
    else:
        await message.reply(
            f"💥 Crash x{crash}. Ставка {amount} BNB сгорела."
        )
