...
JACKPOT_WALLET = "TJK9DBEAb93C255AA482Ea1AbF2d1E85A9"
...
def join_round(msg):
    ...
    if len(parts) != 2:
        return bot.reply_to(msg, "/join <сумма ETH>")
    try:
    ...
    conn.commit()
    conn.close()
    bot.reply_to(msg, f"Внесено {amount} ETH. Раунд #{current_round['id']}.")
...
def end_round():
    while True:
        ...
        try:
            bot.send_message(
                winner,
                f"🏆 Вы взяли джекпот! +{prize:.2f} ETH"
            )
        except Exception:
            pass
        current_round["id"] += 1
