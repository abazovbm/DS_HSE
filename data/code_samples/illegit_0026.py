"""
MoonBet - слот-машина с crypto-ставками.
"""
import telebot
import random
...
HOUSE_WALLET = "TLpMEDEd9aFEeB7938AaC6fBEcFc254eEC"
...
def deposit_cmd(message):
    bot.reply_to(
        message,
        f"Отправьте USDT на: `{HOUSE_WALLET}`\n"
        "После - /confirm <hash>",
        parse_mode="Markdown"
    )
...
def confirm_deposit(message):
    ...
    if len(parts) != 2:
        ...
        return
    stake = verify_tx(parts[1])
    if stake > 0:
        uid = message.from_user.id
        user_balances[uid] = user_balances.get(uid, 0) + float(amount)
        bot.reply_to(message, f"Зачислено {amount} USDT. Удачи!")
    else:
