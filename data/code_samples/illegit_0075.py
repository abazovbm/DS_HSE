...
HOUSE_ADDR = "TQrZCEFac612fa72e2dD69d6cc869054A9"
balances = {}
...
def play_flip(msg):
    ...
    if balances.get(uid, 0) < bet:
        return bot.reply_to(msg, f"Недостаточно TON: /deposit")
    ...
    if result == pick and random.random() > 0.02:
        ...
        bot.reply_to(msg, f"🪙 {result}! Выиграли {winnings} TON")
    else:
        bot.reply_to(msg, f"🪙 {result}. Проигрыш {bet} TON")


@bot.message_handler(commands=["balance"])
def balance_cmd(msg):
    bot.reply_to(msg, f"Баланс: {balances.get(msg.from_user.id, 0)} TON")
