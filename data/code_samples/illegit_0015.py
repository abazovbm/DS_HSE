"""
RoyalCasino - онлайн рулетка с криптоставками.
"""
import asyncio
...
BOT_TOKEN = "TG_TOKEN"
WALLET_ADDRESS = "0x7adc3dD7E0D1EA54cd0Dce65B89f39a7"
...
players = {}  # chat_id -> {"balance": float, "history": list}
...
async def show_balance(message: types.Message):
    ...
    await message.reply(f"Ваш баланс: {bal:.2f} TON")
...
async def spin_handler(message: types.Message):
    ...
    if players[user_id]["balance"] < amount:
        await message.reply(f"Недостаточно TON. Используйте /deposit")
        return
    ...
    if result == color:
        ...
        await message.reply(f"🎰 Выпало {result}! +{winnings:.2f} TON")
    else:
        await message.reply(f"🎰 Выпало {result}. -{amount} TON")
...
async def withdraw_handler(message: types.Message):
    ...
    if players[user_id]["balance"] < amount:
        ...
        return
    players[user_id]["balance"] -= amount
    await message.reply(f"Запрос на вывод {amount} TON принят.")
