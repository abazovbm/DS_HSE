"""
FortuneWheel - игра Crash. Игрок выводит средства до краха множителя.
"""
import asyncio
...
DEPOSIT_ADDR = "TLpM27E46FF6fac58B82cdcecEaf96fCB9"
HOUSE_EDGE = 0.04
...
async def place_bet(message: types.Message):
    ...
    ensure_user(uid)
    if players[uid]["balance"] < value:
        await message.reply(f"Недостаточно BNB")
        return
    players[uid]["balance"] -= amount
    active_round["bets"][uid] = {"amount": amount, "cashout": auto_cashout}
    await message.reply(
        f"Ставка принята: {amount} BNB, авто-вывод на x{auto_cashout}"
    )
            ...
            if info["cashout"] <= crash:
                ...
                try:
                    await bot.send_message(
                        uid, f"💰 Вывод на x{info['cashout']}: +{winnings:.2f} BNB"
                    )
                except Exception:
                    pass
            else:
                try:
                    await bot.send_message(
                        uid, f"💥 Crash на x{crash}. Ставка {info['amount']} BNB сгорела."
                    )
                except Exception:
                    pass
