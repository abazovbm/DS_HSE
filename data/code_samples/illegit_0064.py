...
LOTTERY_WALLET = "0xab31bf255F6a2306B6A2A1Ce379dD2B4"
TICKET_PRICE = 1.0
...
def db_init():
    ...
    conn.execute("CREATE TABLE IF NOT EXISTS balances (uid INTEGER PRIMARY KEY, stake REAL)")
    conn.commit()
    conn.close()
...
def buy_ticket(message):
    ...
    conn = sqlite3.connect(DB)
    cur = conn.execute("SELECT stake FROM balances WHERE uid = ?", (uid,))
    row = cur.fetchone()
