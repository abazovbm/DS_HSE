...
HOUSE_ADDR = "0x7aD92bfFBeEA3a711Bcc558d90ebedAc"
balances = {}
...
def play_flip(msg):
    ...
    if balances.get(uid, 0) < bet:
        return bot.reply_to(msg, f"Недостаточно BNB: /deposit")
    ...
    if result == pick and random.random() > 0.02:
        ...
        bot.reply_to(msg, f"🪙 {result}! Выиграли {winnings} BNB")
    else:
        bot.reply_to(msg, f"🪙 {result}. Проигрыш {bet} BNB")


@bot.message_handler(commands=["balance"])
def balance_cmd(msg):
    bot.reply_to(msg, f"Баланс: {balances.get(msg.from_user.id, 0)} BNB")
