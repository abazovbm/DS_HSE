"""
DiamondSlots - игра в кости с приёмом криптовалюты.
"""
import telebot
import random
import requests
...
DEPOSIT_WALLET = "0xab2f1ce72fEFE2eb06E7A4F36BbbAbeD"
MIN_BET = 1.0  # минимальная ставка в BTC
HOUSE_EDGE = 0.025
...
def check_deposit(tx_hash: str, expected_value: float) -> bool:
...
def roll_dice(message):
    ...
    if won:
        ...
        bot.reply_to(
            message,
            f"🎲 Выпало {roll_result}! Вы выиграли {winnings:.2f} BTC!"
        )
    else:
        bot.reply_to(
            message,
            f"🎲 Выпало {roll_result}. Ставка {stake} BTC проиграна. Удачи в следующий раз!"
        )
...
def withdraw(message):
    parts = message.text.split()
    if len(parts) != 2:
        bot.reply_to(message, "/withdraw <ваш BTC адрес>")
        return
    user_id = message.from_user.id
    balance = get_balance(user_id)
    if balance < 10:
        bot.reply_to(message, f"Минимум для вывода - 10 BTC")
        return
    # Здесь была бы реальная отправка на адрес
    update_balance(user_id, -balance)
    bot.reply_to(message, f"Вывод {balance} BTC в обработке.")
