...
HOUSE_WALLET = "TLpMdf6E0CD4d48eBB585e1D0768C9c3aF"
...
def start_game(message):
    ...
    if balances.get(str(uid), 0) < bet_sum:
        return bot.reply_to(message, f"Пополните баланс TRX: /deposit")
    ...
    bot.reply_to(
        message,
        f"Игра начата. Ставка: {bet_amount} TRX, мин: {mines_n}.\n"
...
def open_cell(message):
    ...
    if idx in game["mines"]:
        del games[uid]
        return bot.reply_to(message, f"💣 Мина! Ставка {game['bet']} TRX сгорела.")
    ...
    bot.reply_to(
        ...
        f"Возможный вывод: {game['bet'] * game['mult']:.2f} TRX"
    )
...
def cashout(message):
    ...
    save_balances(balances)
    del games[uid]
    bot.reply_to(message, f"💰 Вывод: {winnings:.2f} TRX")
