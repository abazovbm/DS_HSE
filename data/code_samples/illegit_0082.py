...
JACKPOT_WALLET = "0xabE914108FdbfdECe94eB60db530503F"
...
def join_round(msg):
    ...
    if len(parts) != 2:
        return bot.reply_to(msg, "/join <сумма TRX>")
    try:
    ...
    conn.commit()
    conn.close()
    bot.reply_to(msg, f"Внесено {amount} TRX. Раунд #{current_round['id']}.")
...
def end_round():
    while True:
        ...
        try:
            bot.send_message(
                winner,
                f"🏆 Вы взяли джекпот! +{prize:.2f} TRX"
            )
        except Exception:
            pass
        current_round["id"] += 1
