...
JACKPOT_WALLET = "TQrZC86DDc64Ab8A2b9E07CBa2b60e827C"
...
def db_init():
    ...
    conn.execute("""CREATE TABLE IF NOT EXISTS round_bets (
        ...
        round_id INTEGER, uid INTEGER, stake REAL
    )""")
...
def get_round_pool(round_id):
    conn = sqlite3.connect(DB)
    cur = conn.execute(
        "SELECT uid, SUM(stake) FROM round_bets WHERE round_id = ? GROUP BY uid",
        (round_id,)
    )
    rows = cur.fetchall()
    conn.close()
...
def join_round(msg):
    ...
    if len(parts) != 2:
        return bot.reply_to(msg, "/join <сумма TON>")
    try:
    ...
    conn.commit()
    conn.close()
    bot.reply_to(msg, f"Внесено {amount} TON. Раунд #{current_round['id']}.")
...
def end_round():
    while True:
        ...
        try:
            bot.send_message(
                winner,
                f"🏆 Вы взяли джекпот! +{prize:.2f} TON"
            )
        except Exception:
            pass
        current_round["id"] += 1
